...
const AddonIcons = () => {
  ...
  return { getIconForAddon };
};

export const AddonIcon = ({ type, name, size = 'medium', selected = false, onClick }) => {
  const { getIconForAddon } = AddonIcons();
  
  const sizeClasses = {
    small: 'addon-icon-small',
    medium: 'addon-icon-medium',
    large: 'addon-icon-large'
  };
  
  const icon = getIconForAddon(type, name);
  
  return (
    <div 
      className={`addon-icon ${sizeClasses[size]} ${selected ? 'selected' : ''}`}
      onClick={onClick}
      title={name || type}
    >
      <span className="addon-emoji">{icon}</span>
      {(name || type !== 'sprinkles') && (
...
export const AddonIconGrid = ({ type, options = [], selected = [], onToggle, title }) => {
  const { getIconForAddon } = AddonIcons();
  
  return (
    <div className="addon-icon-grid">
      {title && <h4 className="addon-grid-title">{title}</h4>}
      <div className="addon-icons-container">
        {options.map((option, index) => {
          const isSelected = selected.includes(option.name);
          return (
            <div 
              key={index}
              className={`addon-icon-item ${isSelected ? 'selected' : ''}`}
              onClick={() => onToggle(option.name, !isSelected)}
            >
              <div className="addon-icon-emoji">
                {
# Создаем новый ProductModal.js с вкладками как в Winners
cat > /opt/metodika-coffee/frontend/src/ProductModal.js << 'EOF'
